...
すなわち、書き込んだ値はそのうち反映されればよいと考え(結果整合性)、その代わりにスケールアウトさせることで非常に高いパフォーマンスを実現することが可能になっています。

etcdは高可用性と強い一貫性を実現するために、Raftという分散合意(Consensus)アルゴリズムに基づいて実装がおこなわれています。
Raftは、Paxosと比べて理解しやすい。リーダー選出、ログのレプリケーションなど
Raftは理論的にも正しいことが証明されており、リーダーがクラスタ内にたかだか1つしか存在しないこと、ログの順序、データが不整合を起こさないこと、信頼性が高い。
